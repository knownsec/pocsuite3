...
from pocsuite3.thirdparty.scapy import base_classes
from pocsuite3.thirdparty.scapy.consts import DARWIN, WINDOWS, LINUX, BSD, SOLARIS
from pocsuite3.thirdparty.scapy.error import log_scapy, warning, ScapyInvalidPlatformException
from pocsuite3.thirdparty.scapy import six
# from themes import NoTheme, apply_ipython_style
...
def _set_conf_sockets():
    ...
    """
    from pocsuite3.thirdparty.scapy.scapy_main import _load
    ...
    if conf.use_pcap:
        try:
            from pocsuite3.thirdparty.scapy.pcapdnet import L2pcapListenSocket, L2pcapSocket, \
                L3pcapSocket
        except (OSError, ImportError):
            logger.info("No libpcap provider available ! pcap won't be used")
            ...
            return
    if conf.use_bpf:
        from pocsuite3.thirdparty.scapy.arch_bpf_supersocket import L2bpfListenSocket, \
        ...
        return
    if LINUX:
        from pocsuite3.thirdparty.scapy.linux import L3PacketSocket, L2Socket, L2ListenSocket
        ...
        return
    if WINDOWS:
        # from arch_windows_init import _NotAvailableSocket
        from pocsuite3.thirdparty.scapy.native import L3WinSocket, L3WinSocket6
        ...
        return
    from pocsuite3.thirdparty.scapy.supersocket import L3RawSocket
    from pocsuite3.thirdparty.scapy.inet6 import L3RawSocket6
...
class Conf(ConfClass):
    ...
    def __getattr__(self, attr):
        ...
        if attr == "manufdb":
            from pocsuite3.thirdparty.scapy.data import MANUFDB
            return MANUFDB
        if attr == "ethertypes":
            from pocsuite3.thirdparty.scapy.data import ETHER_TYPES
            return ETHER_TYPES
        if attr == "protocols":
            from pocsuite3.thirdparty.scapy.data import IP_PROTOS
            return IP_PROTOS
        if attr == "services_udp":
            from pocsuite3.thirdparty.scapy.data import UDP_SERVICES
            return UDP_SERVICES
        if attr == "services_tcp":
            from pocsuite3.thirdparty.scapy.data import TCP_SERVICES
            return TCP_SERVICES
        if attr == "iface6":
