...
from pocsuite3.thirdparty.scapy.automaton import SelectableObject
from pocsuite3.thirdparty.scapy.common import _select_nonblock
from pocsuite3.thirdparty.scapy.compat import raw, plain_str
from pocsuite3.thirdparty.scapy.config import conf
from pocsuite3.thirdparty.scapy.consts import WINDOWS
from pocsuite3.thirdparty.scapy.data import MTU, ETH_P_ALL
from pocsuite3.thirdparty.scapy.pton_ntop import inet_ntop
from pocsuite3.thirdparty.scapy.supersocket import SuperSocket
from pocsuite3.thirdparty.scapy.error import Scapy_Exception, log_loading, warning
from pocsuite3.thirdparty.scapy import consts
...
if conf.use_pcap:
    if WINDOWS:
        ...
        from pocsuite3.thirdparty.scapy.winpcapy import pcap_setmintocopy
    else:
        from pocsuite3.thirdparty.scapy.winpcapy import pcap_get_selectable_fd
    ...
    try:
        from pocsuite3.thirdparty.scapy.winpcapy import PCAP_ERRBUF_SIZE, pcap_if_t, \
        ...
        if WINDOWS:
            ...
            if b"winpcap" in version.lower():
                if os.path.exists(NPCAP_PATH + "\\wpcap.dll"):
                    logger.info("Winpcap is installed over Npcap. "
                            "Will use Winpcap (see 'Winpcap/Npcap conflicts' "
                            "in Scapy's docs)")
                elif platform.release() != "XP":
                    logger.info("WinPcap is now deprecated (not maintained). "
